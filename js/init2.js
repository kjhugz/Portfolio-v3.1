...
jQuery(document).ready(function(){
	...
	kshv_j_modalbox_portfolio();
	kshv_j_imgtosvg();
	kshv_j_popup();
	kshv_j_data_images();
	kshv_j_page_transition();
	kshv_j_projects();
	kshv_j_portfolio();
	kshv_j_owl_carousel();
	kshv_j_section_top();
	kshv_j_background_effects();
	kshv_j_borders();
	kshv_j_canvas_effect();
...
function kshv_j_modalbox(){
	"use strict";
	
	jQuery('.kshv_j_all_wrap').prepend('<div class="kshv_j_modalbox"><div class="box_inner"><div class="close"><a href="#"><i class="icon-cancel"></i></a></div><div class="description_wrap"></div></div></div>');
...
function kshv_j_trigger_menu(){
	
	"use strict";

	var hamburger 		= jQuery('.kshv_j_topbar .trigger .hamburger');
	var mobileMenu		= jQuery('.kshv_j_mobile_menu');
	var mobileMenuList	= jQuery('.kshv_j_mobile_menu ul li a');
	...
	mobileMenuList.on('click',function(){
		jQuery('.kshv_j_topbar .trigger .hamburger').removeClass('is-active');
		mobileMenu.removeClass('opened');
		return false;
	});
}
...
function kshv_j_modalbox_news(){
	...
	var modalBox		= jQuery('.kshv_j_modalbox');
	var button			= jQuery('.kshv_j_news .kshv_j_full_link, .kshv_j_news .news_list .title a, .kshv_j_news .kshv_j_button a');
...
function kshv_j_modalbox_portfolio(){
	
	"use strict";
	
	var modalBox	= jQuery('.kshv_j_modalbox');
	var button		= jQuery('.kshv_j_portfolio .portfolio_popup');
...
function kshv_j_my_load(){
	
	"use strict";
	
	var speed	= 500;
	setTimeout(function(){kshv_j_preloader();},speed);
	setTimeout(function(){jQuery('.kshv_j_all_wrap').addClass('ready');},speed+2000);
}
...
function kshv_j_page_transition(){
	
	"use strict";
	
	var section 		= jQuery('.kshv_j_section');
	var allLi 			= jQuery('.transition_link li');
	var button			= jQuery('.transition_link a');
	var wrapper 		= jQuery('.kshv_j_all_wrap');
	...
	button.on('click',function(){
		...
		if(element.parent().hasClass('kshv_j_button')){
...
function kshv_j_portfolio(){
    ...
    if(jQuery().isotope) {
        ...
        if(filter.length){
            // Initialize Isotope with the featured filter
            list.isotope({
                filter: '.featured', // Only show featured items on load
                animationOptions: {
                    duration: 750,
                    easing: 'linear',
                    queue: false
...
function kshv_j_owl_carousel(){

	"use strict";

	var carousel			= jQuery('.kshv_j_testimonials .owl-carousel');
...
function kshv_j_section_top(){
	"use strict";
	
	var button	= jQuery('.kshv_j_sidebar_menu .menu ul li a,.kshv_j_mobile_menu .menu_list ul li a');
	var section = jQuery('.kshv_j_section');
...
function kshv_j_background_effects(){
	"use strict";
	
	var box		= jQuery('.kshv_j_background_effects');
	var wrapper	= jQuery('.kshv_j_background_effects').data('style');
...
function kshv_j_borders(){
	"use strict";
	jQuery('.kshv_j_mainpart').append('<span class="left_border"></span><span class="right_border"></span><span class="top_border"></span><span class="bottom_border"></span>');
}
